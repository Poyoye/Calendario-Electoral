...
procesos = [
    ...
    ("Artículo 44", "Jornada Electoral y Mesas", "Sorteo y gestión de mesas", 13, "convocatoria"),
...
def calcular_calendario(fecha_convocatoria=None, fecha_elecciones=None):
    ...
    for articulo, proceso, descripcion, dias, referencia in procesos:
        if referencia == "convocatoria":
            ...
            fecha_inicio = fecha_elecciones + timedelta(days=dias)
            fecha_fin = fecha_elecciones + timedelta(days=dias)
        elif referencia == "candidaturas firmes":
            fecha_inicio = fecha_convocatoria + timedelta(days=20 + 2 + 2)  # Tras resolución de recursos
...
def generar_gantt(calendario):
    ...
    gantt_chart = alt.Chart(calendario).mark_bar().encode(
        ...
        color=alt.Color("Artículo:N", title="Artículo", scale=alt.Scale(domain=["Artículo 32", "Artículo 36", "Artículo 39", "Artículo 41", "Artículo 42", "Artículo 44", "Artículo 45", "Artículo 46", "Artículo 49", "Artículo 50"],
